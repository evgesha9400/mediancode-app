<script lang="ts">
	...
	let namespacedFields = $derived(
		workflow.editedItem ? fields.filter((f) => f.namespaceId === workflow.editedItem!.namespaceId) : []
	);
		...
		afterCreate: (field) => {
			...
			workflow.editedItem = {
				...
				members: transitionObjectMembership(
					...
					{
						sourceObjectName: workflow.editedItem.name,
...
<MainColumnFrame bodyClass="">
	{#snippet header()}
		<PageHeader title="Objects">
			{#snippet actions()}
				<NamespaceSelector />
				<button type="button" onclick={workflow.openCreate} class={dashboardPageHeaderPrimaryButton}>
	...
	<Table isEmpty={filteredObjects.length === 0}>
		{#snippet header()}
			<tr>
				<SortableColumn column="name" label="Object Name" {sorts} onSort={workflow.handleSort} />
				<SortableColumn column="description" label="Description" {sorts} onSort={workflow.handleSort} />
				<SortableColumn column="members" label="Members" {sorts} onSort={workflow.handleSort} />
				<SortableColumn column="usedInApis" label="Used In APIs" {sorts} onSort={workflow.handleSort} />
			</tr>
		{/snippet}
		...
		{#snippet body()}
			{#each filteredObjects as object}
				<tr
					...
					class="{tableListRowInteractive} {workflow.isSelected(object) ? tableListRowSelected : tableListRowHover}"
				>
