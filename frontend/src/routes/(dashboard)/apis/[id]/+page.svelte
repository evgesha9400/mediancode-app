<script lang="ts">
	...
	import { newTempMemberId, transitionObjectMembership } from '$lib/domain/objectMembership';
	import {
		BackNavButton,
		MainColumnFrame,
		DrawerStack,
	...
	const availableFields = $derived($fieldsStore.filter((f) => f.namespaceId === apiState.apiNamespaceId));
	const availableObjects = $derived($objectsStore.filter((o) => o.namespaceId === apiState.apiNamespaceId));
	...
	let namespaceName = $derived($namespacesStore.find((ns) => ns.id === apiState.apiNamespaceId)?.name ?? '');
		...
		createFieldsContract({
			...
			afterCreate: (field) => {
				...
				objectWorkflow.editedItem = {
					...objectWorkflow.editedItem,
					members: transitionObjectMembership(
						objectWorkflow.editedItem.members,
						{ type: 'fieldMemberAdded', fieldId: field.id },
						{
							sourceObjectName: objectWorkflow.editedItem.name,
	...
	<MainColumnFrame bodyClass="p-6">
		...
		<div class="flex min-h-[50vh] flex-col items-center justify-center">
			<div class="text-center">
				...
				<button type="button" onclick={() => goto('/apis')} class={dashboardPageHeaderPrimaryButton}>
					Back to APIs
				</button>
			</div>
		</div>
...
{:else}
	<MainColumnFrame bodyClass="">
		{#snippet header()}
			<div class={dashboardPageHeaderShell}>
				<div class="relative z-20 flex justify-between gap-3 {dashboardPageHeaderTitleBand} flex-nowrap">
					<div class="flex items-center gap-2 sm:gap-3 min-w-0 flex-1 overflow-hidden">
						<BackNavButton ariaLabel="Back to APIs list" onclick={() => goto('/apis')} />
						...
						{#if apiState.api?.description?.trim()}
							...
							<span
								...
								title={apiState.api.description}>{apiState.api.description.trim()}</span
							>
						{/if}
							...
							>
								<div class="flex items-center space-x-2">
									...
									<span class="text-xs text-fg-muted"
										>{section.endpoints.length} endpoint{section.endpoints.length !== 1 ? 's' : ''}</span
									>
								</div>
								<i
									class="fa-solid fa-chevron-down text-fg-muted text-sm transition-transform {isExpanded
										? 'rotate-0'
										: '-rotate-90'}"
								></i>
							</button>
								...
								<div class="p-4">
									<div class="space-y-2">
										{#each section.endpoints as endpoint (endpoint.id)}
											<EndpointItem {endpoint} onClick={() => apiState.openEndpoint(endpoint)} />
										{/each}
									</div>
								</div>
							{/if}
	...
	{#snippet editApiFormContent(_: { close: () => void })}
		<div class="space-y-4">
			...
			<!-- Version -->
			<FormField id="edit-version" label="Version" bind:value={apiState.editForm.version} placeholder="1.0.0" />
			...
			<!-- Base URL -->
			<FormField id="edit-base-url" label="Base URL" bind:value={apiState.editForm.baseUrl} placeholder="/api/v1" />
		</div>
	{/snippet}
		...
		{#if !apiState.showEditDeleteConfirm}
			<div class={drawerFooterSegmentedPanel} role="group" aria-label="Edit API actions">
				<button
					...
					class="{drawerFooterSegmentBtn} font-medium {drawerFooterBtnDestructive} {apiState.isSaving
					...
					<div class="relative">
						...
						<div class="relative">
							<input
								...
								onblur={() =>
									setTimeout(() => {
										apiState.tagDropdownOpen = false;
									}, 150)}
								onkeydown={(e) => {
									if (e.key === 'Enter') {
										e.preventDefault();
							...
							{:else}
								<i
									class="fa-solid fa-chevron-down absolute right-3 top-1/2 -translate-y-1/2 text-fg-muted text-xs pointer-events-none"
								></i>
							{/if}
						</div>
								...
								<div class="{dropdownListScroll} max-h-48">
									...
									{#each filteredTags as tag (tag)}
										<button
											...
											class="{dropdownRow} text-sm text-fg-secondary {apiState.editedEndpoint?.tagName === tag
												? 'bg-surface-raised'
												: ''}"
										>
											{tag}
					...
					{#if apiState.editedEndpoint.pathParams.length === 0}
						<div class="px-3 py-2 {surfaceInsideFrostedPanel}">
							<p class="text-xs text-fg-muted">
								No path parameters. Add parameters to your URL path using <code
									class="bg-surface-raised px-1 rounded-lg">{`{param_name}`}</code
								>
							</p>
						</div>
					{:else}
						<div class="px-3 py-1 {surfaceInsideFrostedPanel}">
							<!-- Column headers -->
							<div
								class="flex items-center gap-2 py-1 border-b border-edge text-[10px] text-fg-dimmed uppercase tracking-wider"
							>
							...
							{#each apiState.editedEndpoint.pathParams as param (param.name)}
								<ParameterEditor
									...
									validationErrors={apiState.validationErrors.filter(
										(e) => e.location?.kind === 'pathParam' && e.param === param.name
									)}
									suggestions={endpointQuerySuggestions}
									onFieldSelect={(fieldMemberId) => apiState.handlePathParamFieldSelect(param.name, fieldMemberId)}
								/>
							{/each}
						</div>
					{/if}
			...
			{#if apiState.isCreating}
				<div class={drawerFooterSegmentedPanel} role="group" aria-label="Create endpoint actions">
					<button
						...
						disabled={apiState.isSaving}
						class="{drawerFooterSegmentBtn} {drawerFooterBtnSecondarySegment} {apiState.isSaving
							? 'cursor-not-allowed opacity-50'
							: ''}"
					>
						<span>Cancel</span>
					...
					<button
						...
						class="{drawerFooterSegmentBtn} {apiState.hasEndpointChanges &&
						!apiState.endpointCommandBlocked &&
						!apiState.isSaving
							? drawerFooterBtnPrimaryEnabled
							: drawerFooterBtnPrimaryDisabledSegment}"
					>
			...
			{:else if !apiState.showEndpointDeleteConfirm}
				<div class={drawerFooterSegmentedPanel} role="group" aria-label="Edit endpoint actions">
					...
					<button
						...
						disabled={apiState.isSaving}
						class="{drawerFooterSegmentBtn} {apiState.isSaving
							? drawerFooterBtnDuplicateSegmentMuted
							: drawerFooterBtnDuplicateSegment}"
					>
						<span>Duplicate</span>
				...
				? [
						{
							id: 'edit-api',
							title: 'Edit API',
							headerNamespace: namespaceName,
							width: 520,
							minWidth: 380,
							content: editApiFormContent,
							footer: editApiFormFooter
						}
					]
				: []),
				...
				? [
						{
							...
							width: 1200,
							minWidth: 700,
							content: endpointFormContent,
							footer: endpointFormFooter
						}
					]
				: []),
